...
fun <T> State<T>.orDefault(recover: (Throwable) -> T): State<T> =
    when (this) {
...
fun <T> State<T>.getOrElse(recover: (Throwable) -> T): T =
    when (this) {
...
fun <T> State<T>.getOrDefault(default: T): T =
    when (this) {
